...
echo [STRM_1.0_1] INFO : Selected arguments : %* |find /I "/GS" && (
    echo [STRM_1.0_1] INFO : /GS option selected.
	goto gs
    )
echo [STRM_1.0_1] INFO : Selected arguments : %* |find /I "/RP" && (
    echo [STRM_1.0_1] INFO : /RP option selected.
	goto rp
    )
echo [STRM_1.0_1] INFO : Selected arguments : %* |find /I "/HP" && (
    echo [STRM_1.0_1] INFO : Displaying help message.
	goto hp
    )
echo [STRM_1.0_1] ERR : Invalid option selected.
endlocal && exit /b 1
:rp
...
echo [STRM_1.0_1] SUCCESS :  Modified string : !string!.
endlocal && exit /b 0
:gs
if not "%~1" == "" set "string=%~1"
